...
## Nachtrag 2026-07-27 (Phase C): `WorkoutPlan.activityType` ergänzt
Beim Bau des Workout-Builders (Phase C) fiel auf, dass `WorkoutPlan` selbst kein `ActivityType`-Feld hatte, obwohl die Aufgabenstellung verlangt, beim Plan-Anlegen die Sportart festzulegen (bestimmt, ob Kraft- oder Cardio-Zielfelder auf den `PlannedExercise`-Einträgen angezeigt werden). Feld wurde direkt in `SchemaV1` ergänzt (`var activityType: ActivityType = .kraft`), **ohne** eine `SchemaV2`-Migrationsstufe zu bauen: Es existieren noch keine echten/veröffentlichten Datenbestände (nur lokale Simulator-Stores während der Entwicklung), eine Migrationsübung dafür wäre reine Demonstration ohne fachlichen Anlass – YAGNI. Die `VersionedSchema`/`MigrationPlan`-Infrastruktur aus ADR 0001 bleibt für den ersten *echten* Migrationsfall (nach einem Zwischenstand mit realen Nutzerdaten) vorbereitet.

## Nachtrag 2026-07-28: Cardio-Metrik-Mechanismus durch Segmente ersetzt
Der hier ursprünglich gewählte flache, optionale Cardio-Metrik-Block (`distance`/`duration`/`averageHeartRate` direkt auf `WorkoutSession`, dieselben `PlannedExercise`-Zielfelder wie Kraft) wurde durch ein eigenes Modellpaar (`PlannedSegment`/`SegmentLog`) abgelöst, siehe ADR 0009. Die Kernentscheidung dieses ADRs (ein `WorkoutSession`-Typ mit `ActivityType` statt paralleler Entity-Hierarchien pro Sportart) bleibt unverändert bestehen - revidiert wurde nur, WIE die Cardio-Metriken innerhalb dieses einen Typs abgebildet werden.
