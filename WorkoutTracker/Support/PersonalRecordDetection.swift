...
extension WorkoutSession {
    ...
    /// Äquivalent (ADR 0009). Zählt alle Arbeitssatz-`SetLog`s dieser Session,
    /// unabhängig vom `isCompleted`-Häkchen (Konsistenz mit
    /// `previousAttempt()`, siehe ADR-0002-Nachtrag Phase D) - Warm-up-Sätze
    /// zählen bewusst nie als Rekord.
    func detectAndPersistPersonalRecords(in context: ModelContext) {
        guard activityType.usesSetLogs else { return }

        let bestThisSessionByExercise = Dictionary(grouping: setLogs.filter { !$0.isWarmup }, by: \.exerciseName)
